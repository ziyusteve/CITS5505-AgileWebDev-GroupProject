...
def visualize(dataset_id):
    ...
    is_shared = Share.query.filter_by(
        dataset_id=dataset_id, shared_with_id=user_id
    ).first() is not None

    if not (is_owner or is_shared):
        flash(
            'You do not have permission to view this dataset.', 'danger'
        )
    ...
    if hasattr(Dataset, 'has_scout_analysis') and callable(
        ...
        if dataset.has_scout_analysis():
            ...
            current_app.logger.warning(
                f"[DEBUG] Scout report analysis: {scout_analysis.__dict__ if scout_analysis else None}"
            )
        ...
        try:
            ...
            if not scout_analysis:
                scout_analysis = ScoutReportAnalysis(
                    ...
                    analysis_result=json.dumps({
                        "summary": "This is an uploaded text file. The system cannot detect detailed analysis results, please check the original file content."
                    })
                )
            ...
            if not scout_analysis.analysis_result:
                scout_analysis.analysis_result = json.dumps({
                    "summary": "The system has received your analysis request but could not generate detailed analysis. This may be due to incompatible file format or content."
                })
                scout_analysis.processing_status = 'completed'
        except Exception as e:
            current_app.logger.error(
                f"Failed to create fallback analysis: {str(e)}"
            )
